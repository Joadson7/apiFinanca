...
public class TipoServiceImpl implements TipoService {
	...
	public TipoResponseDto atualizar(Integer id, TipoRequestDto request) throws Exception {
		...
		var tipo = tipoRepository
				.findById(id)
				.orElseThrow(() -> new IllegalArgumentException("Tipo não encontrada."));
	...
	public List<TipoResponseDto> consultar() throws Exception {
		...
		var contas = tipoRepository.findAll();
		
		return contas
				.stream()
				.map(tipo -> modelMapper.map(tipo, TipoResponseDto.class))
